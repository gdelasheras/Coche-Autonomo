...
public class SpeedoMeterScript : MonoBehaviour 
{
	...
	public float currentValue = 120;						//This value changes dynamically, between the minimum and the cap.	(Change this to your own value, like Amount of fuel, Speed, Chickens gathered etc).
	private float capValue = 240; 							//This is the max value. The cap. (Change this to howmany fuel you can hold, the maximum speed you can travel or the maximum amount of chickens you can gather.
	...
	void Update()
	{
        //this.angle = startingAngle + (((currentValue / startingValue) * percentage) * maximumDegrees);			//Calculation of the Angle in realtime. Replace the currentValue and startingValue with anything you like.
        //Leave the rest as how it is.

        this.angle = (currentValue * 10) - 120;

        //Debug.Log("  angulo  " + (currentValue * 10));
        ...
        digitOne = (int)Mathf.Floor(currentValue * 10 / (Mathf.Pow(10, 2)) % 10);
        digitTwo = (int)Mathf.Floor(currentValue * 10 / (Mathf.Pow(10, 1)) % 10);
        digitThree = (int)Mathf.Floor(currentValue * 10 / (Mathf.Pow(10, 0)) % 10);
